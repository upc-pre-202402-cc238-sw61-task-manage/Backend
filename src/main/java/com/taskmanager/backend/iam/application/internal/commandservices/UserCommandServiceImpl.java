...
import com.taskmanager.backend.iam.application.internal.outboundservices.hashing.HashingService;
import com.taskmanager.backend.iam.application.internal.outboundservices.tokens.TokenService;
...
public class UserCommandServiceImpl implements UserCommandService {
    ...
    public Optional<User> handle(SignUpCommand command) {
        ...
        var roles = command.roles().stream().map(role -> roleRepository.findByName(role.getName()).orElseThrow(() -> new RuntimeException("Role title not found"))).toList();
